...
export default function VerificationPage({ params }: { params: { productId: string, batchId: string } }) {
  ...
  useEffect(() => {
    fetch(`/api/verification/${params.productId}/${params.batchId}`)
      .then((response) => response.json())
      .then((data) => {
        if (data.success) {
          setBatchData(data.batch);
        } else {
          setError(data.message || "Không tìm thấy batch");
        }
      })
      .catch(() => setError("Không thể tải dữ liệu xác minh"))
    ...
    <div className="bg-[#FAF9F6] text-[#2C3830] min-h-screen font-sans selection:bg-[#2C3830] selection:text-white">
      ...
      {loading ? (
        ...
        <div className="min-h-screen flex items-center justify-center px-6 text-center">
          <div>
            <p className="text-sm uppercase tracking-widest text-[#2C3830]/50 mb-3">Verification</p>
            <h1 className="font-serif text-3xl mb-4">{error}</h1>
            <Link href="/shop" className="inline-flex items-center gap-2 bg-[#2C3830] text-white px-6 py-3 rounded-xl">
              <ChevronLeft size={16} />
              Quay lại cửa hàng
            </Link>
          </div>
        ...
        >
          ...
          <motion.section variants={fadeIn} className="mb-20">
              ...
              {batchData.mediaType === 'video' ? (
                <video 
                  src={batchData.mediaUrl} 
                  autoPlay 
                  loop 
                  muted 
                  playsInline
